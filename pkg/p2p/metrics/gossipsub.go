...
var (
	totalPeers       = NewGauge("total_peers", "Total number of peers", nil)
	peersPerProtocol = NewGauge("peers_per_protocol", "Number of peers per protocol", []string{"protocol"})
)

var (
	// ProcessedMessagesDuration in nanoseconds to process a message. Labeled by protocol and result.
	ProcessedMessagesDuration = NewHistogramWithBuckets(
